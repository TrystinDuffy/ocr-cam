...
import {
  ...
  type FrameScheduler,
} from "../camera/frameScheduler.js";
import { createOcrView, type OcrView } from "../view/OcrView.js";
...
export function createOcrController(
  ...
  const state: StateMachine = createStateMachine();
  const events = new TypedEventTarget<OcrEventMap>();
  const cameraCtrl: CameraController = createCameraController({
    onTrackEnded: handleTrackEnded,
  });
  const frameScheduler: FrameScheduler = createFrameScheduler(
    config.inference?.maxFps ?? 8
  );

  let currentEngine: OcrEngine | null = engine ?? null;
  let loadPromise: Promise<void> | null = null;
  let sessionId = 0;
  let frameId = 0;
  let inferenceBusy = false;
  let currentCropConfig: CropConfiguration | null = config.crop ?? null;
  let resolvedCrop: ResolvedCropRegion | null = null;
  let destroyed = false;
  ...
  let lastTemporalResult: OcrResult | null = null;

  let prevInputForSmoothing: ImageData | null = null;
  ...
  type Det = OcrResult["detections"][number];
  type Box = Det["box"];

  function rectIoU(a: Box, b: Box): number {
    const ax1 = a.x;
    const ay1 = a.y;
  ...
  function weightedAvgBox(dets: Det[]): Box {
    let sumW = 0;
    let x = 0;
    let y = 0;
    let w = 0;
  ...
  function smoothDetectionsFromBuffer(buffer: OcrResult[], boxIouThreshold: number): Det[] {
    ...
    const smoothed: Det[] = anchors
      .map((anchor) => {
        ...
        if (members.length === 0) members.push(anchor);

        const box = weightedAvgBox(members);
        const confidence =
          members.reduce((acc, d) => acc + d.confidence, 0) / Math.max(1, members.length);
  ...
  function applyInputTemporalSmoothing(imageData: ImageData, alpha: number): void {
    ...
    if (
      prevInputForSmoothing.width !== imageData.width ||
  ...
  function applyContrastStretch(imageData: ImageData): void {
    ...
    if (!(range > 10)) return; // avoid extreme noise amplification
  ...
  let reuseCanvas: OffscreenCanvas | null = null;
  let reuseCtx: OffscreenCanvasRenderingContext2D | null = null;
  ...
  function computeActions(): OcrActionState {
  ...
  function updateCropForSource(width: number, height: number): void {
  ...
  function ensureCanvas(w: number, h: number): OffscreenCanvasRenderingContext2D {
    ...
    if (!reuseCtx) throw new Error("Failed to create OffscreenCanvas 2D context");
    return reuseCtx;
  }

  async function processFrame(timestamp: number): Promise<void> {
    if (state.getState() !== "running") return;
      ...
      if (crop) {
        ...
        if (temporalCfg) {
          ...
          if (typeof alpha === "number" && alpha > 0 && alpha < 1) {
            applyInputTemporalSmoothing(frameData.imageData as ImageData, alpha);
          }
          if (temporalCfg.contrastStretch) {
            applyContrastStretch(frameData.imageData as ImageData);
          }
        }

        const context: OcrFrameContext = {
          sessionId,
      ...
      const combinedText = detections.map((det) => det.text).join("\n");
       ...
       if (temporalCfg) {
         ...
         if (windowSize > 1) {
           ...
           while (temporalBuffer.length > windowSize) temporalBuffer.shift();

           const boxIouThreshold = temporalCfg.boxIouThreshold ?? 0.5;
           const smoothed = smoothDetectionsFromBuffer(
             temporalBuffer,
             boxIouThreshold
           );
  ...
  const controller: OcrController = {
    ...
    async startCamera(options?: CameraStartOptions) {
      assertNotDestroyed();
      ...
      if (state.getState() !== "ready") {
        throw new OcrError(
          "INVALID_STATE",
          `Cannot start camera in state "${state.getState()}" – call load() first`
        );
      }
    ...
    attachView(container: HTMLElement, options?: ViewOptions) {
      assertNotDestroyed();
      if (state.getState() !== "running") {
        throw new OcrError(
          "INVALID_STATE",
          `Cannot attach view in state "${state.getState()}" – camera must be running`
        );
      }
    ...
    setCrop(crop: CropConfiguration | null) {
      assertNotDestroyed();
